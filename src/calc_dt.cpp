...
//	Kokkos::MDRangePolicy <Kokkos::Rank<2>> policy({x_min + 1, y_min + 1}, {x_max + 2, y_max + 2});
    ...
    clover::Range2d r(xStart, yStart, xEnd, yEnd);


    dt_min_val = tbb::parallel_reduce(
            tbb::blocked_range2d<size_t>{r.fromY, r.toY, r.fromX, r.toX}, g_big,
            [&](const tbb::blocked_range2d<size_t> &br, double acc) {
                for (size_t j = br.rows().begin(); j < br.rows().end(); ++j) {
                    for (size_t i = br.cols().begin(); i < br.cols().end(); ++i) {
                        ...
                        double mins = std::fmin(dtct,
                                                std::fmin(dtut, std::fmin(dtvt, std::fmin(dtdivt,
                                                                                          g_big))));
                        acc = std::fmin(acc, mins);
                    }
                }
                return acc;
            }, [](auto l, auto r) { return std::fmin(l, r); }, partitioner);
